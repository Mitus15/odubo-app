    ...
    >
      ...
      <div className="px-4 py-6 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 80px)' }}>
        {loading ? (
          ...
          <p className="text-center text-white/50">No links available</p>
        ) : (
          <div className="space-y-3 max-w-md mx-auto">
            {links.map((link) => (
              <button
                ...
                className="w-full flex items-center gap-4 p-4 rounded-xl bg-white/10 text-white text-left active:bg-white/20"
              >
...
function PlatformIcon({ platform }: { platform: string | null }) {
  switch (platform) {
    ...
    case 'apple_music':
      return (
        <svg viewBox="0 0 24 24" className="w-5 h-5" fill="#FA2D48">
