  ...
  return (
    <main className="mx-auto flex min-h-[100dvh] max-w-md flex-col items-center gap-4 bg-ink px-5 py-6 text-bone">
      <header className="text-center">
        <h1 className="text-2xl font-extrabold text-electric">Pose Studio</h1>
        <p className="text-sm opacity-70">Strike a pose — we&apos;ll Loop Soul it.</p>
      </header>

      <div className="flex w-full rounded-full border border-electric/25 bg-ink-soft p-1">
        {TABS.map((t) => (
          <button
            key={t.id}
            type="button"
            ...
            className={`flex-1 rounded-full py-2 text-sm font-bold transition-colors ${
              mode === t.id ? "bg-electric text-ink" : "text-bone/70"
            }`}
          >
            {t.label}
          </button>
