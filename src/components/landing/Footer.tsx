...
export default function Footer() {
  ...
  useEffect(() => {
    fetch('/api/linktree')
      .then(res => res.json())
      .then(data => {
        const allLinks = (data as { links?: LinkTreeItem[] }).links || [];
        // Filter out store links
        const filtered = allLinks.filter((link: LinkTreeItem) => 
          link.platform !== 'shopify' && link.platform !== 'odubo'
        );
        setLinks(filtered);
        setLoading(false);
      })
  ...
  return (
    <footer ref={ref} className="fixed bottom-0 left-0 right-0 z-30 bg-[#0d0c0a]/90 border-t border-[#502d26]/10 hidden lg:block backdrop-blur-md">
      <div className="max-w-4xl mx-auto px-6 py-3">
        <div className="flex items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-4 flex-1">
            ...
            <form onSubmit={handleSubmit} className="flex items-center gap-2 flex-1 max-w-xs">
              <input
                type="email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder="Join the list"
                className="w-full px-3 py-2 bg-[#1a1816]/80 border border-[#502d26]/30 rounded-lg text-[#ede8df] text-sm placeholder:text-[#ede8df]/40 focus:outline-none focus:border-[#843c2d]/50"
              />
              <button
                type="submit"
                className="px-4 py-2 bg-[#843c2d] text-[#ede8df] text-sm font-medium rounded-lg hover:bg-[#9a4535] transition-colors"
              >
                {submitted ? '✓' : 'Join'}
              </button>
            </form>
          ...
          {/* Right - Link Icons (same as Connect modal) */}
          <div className="flex items-center gap-1">
            {!loading && links.slice(0, 5).map((link) => (
              <button
                key={link.id}
                onClick={() => handleLinkClick(link.url)}
                className="w-9 h-9 rounded-lg flex items-center justify-center text-[#ede8df]/50 hover:text-[#ede8df] hover:bg-[#843c2d]/20 transition-all"
