...
import { officialCoverKey } from "@/lib/loop/cover";
 ...
 *   official  → this shot becomes the volume's official cover (implies feature)
 *   unofficial→ the volume has no official cover again
...
export async function POST(req: NextRequest) {
  ...
  if (action === "delete") {
    ...
    }>(`SELECT r2_key, thumbnail_key FROM gallery_photos WHERE id = ?1`, [id]);
  ...
  // The official cover is a decision about the volume, not a column on the
  // photo — one value in loop_settings, so there can only ever be one, and
  // "unofficial" is the absence of it rather than a second source of truth.
  if (action === "official" || action === "unofficial") {
    const row = await queryOne<{ uid: string }>(
      `SELECT uid FROM gallery_photos WHERE id = ?1`,
      [id],
    );
    if (!row) return NextResponse.json({ error: "Not found" }, { status: 404 });
    const event = await getCurrentEvent();
    if (action === "official") {
      // Featured too: the official cover must be visible in the room's gallery.
      await executeQuery(
        `UPDATE gallery_photos SET featured = 1, moderated = 1, moderated_at = ?2, moderated_by = 'loop-admin' WHERE id = ?1`,
        [id, now],
      );
      await setSetting(officialCoverKey(event.id), row.uid);
    } else {
      await setSetting(officialCoverKey(event.id), null);
    }
    return NextResponse.json({ success: true, id, action, uid: row.uid });
  }
