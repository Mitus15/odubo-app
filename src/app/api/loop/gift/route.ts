...
export async function POST(req: Request) {
  ...
  if (body.action === "mint") {
    ...
    const code = mintCode();
    await executeQuery(`INSERT INTO loop_gift_codes (code, name) VALUES (?1, ?2)`, [code, name]);
    return NextResponse.json({ code, name });
  }

  if (body.action === "visit") {
    const code = typeof body.code === "string" ? body.code.trim().slice(0, 16) : "";
    const visitor = typeof body.visitor === "string" ? body.visitor.trim().slice(0, 64) : "";
    if (!code || !visitor) return NextResponse.json({ error: "code and visitor required" }, { status: 400 });
