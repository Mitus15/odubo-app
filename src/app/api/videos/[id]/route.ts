  ...
  try {
    ...
    async function deleteAllDescendants(parentId: number): Promise<void> {
      ...
      for (const clip of childClips || []) {
        ...
        // Delete dependent records before deleting the video itself
        console.log(`[DELETE] Deleting dependent records for clip ${clip.id}`);
        try {
    ...
    // Delete dependent records for the parent video before deleting it
    console.log(`[DELETE] Deleting dependent records for parent video ${videoId}`);
    try {
