...
async function handlePayload(payload: any): Promise<NextResponse> {
  ...
  // Check current status to make idempotent
  let existing: any[] = [];
  try {
    existing = await queryDatabase('SELECT status FROM videos WHERE stream_video_id = ? LIMIT 1', [uid]);
  } catch {
    // If column not present, nothing to update
    return NextResponse.json({ success: true, skipped: true });
  }
  if (!existing.length) return NextResponse.json({ success: true });
  ...
  if (typeof duration === 'number') { fields.push('duration = ?'); params.push(String(Math.max(0, Math.floor(duration)))); }
  if (thumbnailUrl) { fields.push('thumbnail = ?'); params.push(thumbnailUrl); }
  params.push(uid);

  await executeQuery(`UPDATE videos SET ${fields.join(', ')} WHERE stream_video_id = ?`, params);
