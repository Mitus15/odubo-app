...
const VideoCard = ({ video, onClick, onVisibilityToggle }: { 
  video: Video; 
  onClick: () => void;
  onVisibilityToggle?: (videoId: number, isLive: boolean) => Promise<void>;
}) => {
  const [toggling, setToggling] = useState(false);
  
  const handleVisibilityClick = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!onVisibilityToggle || toggling) return;
    
    setToggling(true);
    try {
      const isCurrentlyLive = (video as any).publication_status === 'live';
      await onVisibilityToggle(video.id, !isCurrentlyLive);
    } finally {
      setToggling(false);
    }
  };
  
  const isLive = (video as any).publication_status === 'live';
    ...
    >
      ...
      {onVisibilityToggle && (
        <button
          ...
          title={isLive ? 'Hide from public' : 'Make live now'}
        >
          {toggling ? (
            <svg className="w-4 h-4 animate-spin text-[#b2a491]" fill="none" viewBox="0 0 24 24">
            ...
            <svg className="w-4 h-4 text-[#b2a491]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function AdminVideosPage() {
  ...
  const handleVisibilityToggle = async (videoId: number, makeLive: boolean) => {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`/api/admin/videos/${videoId}/visibility`, {
        ...
        body: JSON.stringify({
          publication_status: makeLive ? 'live' : 'archived',
          is_public: makeLive ? 1 : 0,
        }),
      });
