...
curl -X POST "https://api.cloudflare.com/client/v4/zones/$ZONE_ID/firewall/rules" \
  ...
  -d '{"filter":{"expression":"http.request.uri.query contains \"'\''","paused":false},"action":"block","description":"SQL Injection Protection","paused":false}'
...
curl -X POST "https://api.cloudflare.com/client/v4/zones/$ZONE_ID/rate_limit_rules" \
  -H "Authorization: Bearer $API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"description":"API Rate Limiting","expression":"http.request.uri.path contains \"/api/\"","action":"challenge","ratelimit":{"period":60,"requests_per_period":100,"mitigation_timeout":300}}'
